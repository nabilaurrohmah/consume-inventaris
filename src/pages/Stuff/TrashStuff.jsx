...
export default function TrashStuff() {
  ...
  const restoreStuff = (id) => {
    instance.put(`stuffs/restore/${id}`, null, {
        headers: {
          Authorization: "Bearer " + localStorage.getItem("access_token"),
        },
      })
          ...
          <div className="flex mt-4 md:mt-6">
            <table className="min-w-full text-left text-sm font-light">
              ...
              <tbody className="text-black">
                {trashs.map((stuffs, id) => (
                  <tr key={stuffs.id} className="border-b dark:border-neutral-500">
                    <td className="whitespace-nowrap px-6 py-4">{id + 1}</td>
                    <td className="whitespace-nowrap px-6 py-4">{stuffs.name}</td>
                    <td className="whitespace-nowrap px-6 py-4">{stuffs.category}</td>
                    <td className="whitespace-nowrap px-6 py-4">{stuffs.stock ? stuffs.stock.total_available : "0"}</td>
                    <td className="whitespace-nowrap px-6 py-4">{stuffs.stock ? stuffs.stock.total_defec : "0"}</td>
